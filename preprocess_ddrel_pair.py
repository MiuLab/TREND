...
def preprocess(PATH: str, mode: str) -> None:
    ...
    i = 0
    while i < len(data):
        if i > 0 and data[i]['pair-id'] == data[i-1]['pair-id']:
            d = d + ' ' + expand_contractions(' '.join(data[i]['context']))
        else:
        ...
        if i == len(data) - 1 or data[i+1]['pair-id'] != data[i]['pair-id']:
            examples.append({
                ...
                'label': four_cls_map[int(label)]-1,
                't_start': t_start,
                't_end': t_end,
                'trigger': trigger,
                'x_st': x_st,
...
if __name__ == '__main__':
    preprocess('dev.txt', 'dev')
    preprocess('train.txt', 'train')
    preprocess('test.txt', 'test')
